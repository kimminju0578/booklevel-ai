...
export default async function QuizPage({ params }: { params: Promise<{ categorySlug: string }> }) {
  ...
  return <PageShell active="" eyebrow="A MOMENT TO THINK" title={`${category} 분야 진단`} description="다른 사람의 속도보다, 지금의 나에게 집중하세요.">{configured() ? <AssessmentQuiz categorySlug={categorySlug} /> : <DemoAssessment category={category} categorySlug={categorySlug} />}</PageShell>;
}
