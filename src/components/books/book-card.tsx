...
export function BookCover({ book, source, eager = false }: { book: Book; source?: string; eager?: boolean }) {
  const src = source || book.coverUrl || null;
  ...
  return <div className="book-cover">
    ...
    {src && failedSource !== src && <Image src={src} alt={`${book.title} 표지`} fill sizes="(max-width: 600px) 160px, 240px" unoptimized loading={eager ? "eager" : "lazy"} onError={() => setFailedSource(src)} />}
  </div>;
}
