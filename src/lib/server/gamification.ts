...
export async function awardVerifiedActivity(
  userId: string,
  input: VerifiedActivity,
) {
  ...
  try {
    ...
    await event(userId, "rating_earned", {
      eventType: value.eventType,
      points: result?.points ?? 0,
    });
    const streak = checked(
      await adminDb().rpc("record_streak_activity", { p_user: userId }),
    );
...
function progress(rating: number, currentKey: string) {
  ...
  const next =
    RANK_TIERS.slice(index + 1).find((tier) => tier.key !== "challenger") ??
    null;
  const ratio = next
    ? Math.min(
        1,
        Math.max(
          0,
          (rating - current.minRating) /
            Math.max(1, next.minRating - current.minRating),
        ),
      )
    : 1;
...
export async function rankingMe() {
  ...
  const ratingRow = checked(
    await database
      .from("user_ratings")
      .select("rating,tier_key,current_season_rank")
      .eq("user_id", user.id)
      .maybeSingle(),
  );
  const rating = ratingRow?.rating ?? 0;
  const tier = progress(rating, ratingRow?.tier_key ?? "reader_1");
  const season = checked(
    await database
      .from("seasons")
  ...
  const categoryRanks = season
    ? checked(
        await database
          .from("category_rankings")
          .select("category_id,rank,rating,categories(name,slug)")
  ...
  const badges =
    checked(
      await database
        ...
        .select(
          "slot,user_badges(id,earned_at,badges(key,name,description,family,rarity,icon_key,mark_asset_url,full_asset_url))",
        )
        .eq("user_id", user.id)
        .order("slot"),
    ) ?? [];
  const challenger = Boolean(
    season &&
      isChallenger(
        ratingRow?.current_season_rank ?? null,
  ...
  return {
    rating,
    tier: {
      key: challenger ? "challenger" : tier.current.key,
      label: challenger ? "Challenger" : tier.current.label,
      division: challenger ? "Challenger" : tier.current.division,
    },
    nextTier: tier.next
      ? {
          key: tier.next.key,
          label: tier.next.label,
          minRating: tier.next.minRating,
        }
      : null,
    ...
    season: season
      ? {
          id: season.id,
          name: season.name,
          rank: ratingRow?.current_season_rank ?? null,
        }
      : null,
    categoryRanks,
...
export async function ranking(request: Request) {
  ...
  const query = categoryId
    ? database
        .from("category_rankings")
        .select("rank,rating,user_id,profiles(id,display_name,avatar_url)")
        .eq("season_id", season.id)
        .eq("category_id", uuid.parse(categoryId))
        .order("rank")
        .limit(limit)
    : database
  ...
  const viewer = user
    ? (rows.find((row) => row.user_id === user.id) ?? null)
    : null;
  const ratingRows = rows.length
    ? (checked(
        await database
          .from("user_ratings")
          .select("user_id,tier_key,current_season_rank")
          .in(
            "user_id",
  ...
  const mappedRows = rows.map((row) => {
    ...
    return {
      ...
      rankMark: ratingRow
        ? {
            tier: ratingRow.tier_key,
            challengerRank: isChallenger(
              ratingRow.current_season_rank,
              season.status,
            )
              ? ratingRow.current_season_rank
              : null,
  ...
  const viewerRatingRow = user ? ratingByUser.get(user.id) : null;
  return {
    season,
    rows: mappedRows,
    viewer: viewer
      ? {
          ...
          tier: viewerRatingRow?.tier_key ?? null,
        }
      : null,
  };
}
  ...
  return {
    badges: checked(
      await db
        .from("badges")
        .select(
          "id,key,name,description,family,rarity,icon_key,mark_asset_url,full_asset_url,series_key,series_order,criteria",
        )
        .eq("is_active", true)
        .order("family")
        .order("series_order"),
  ...
  const owned =
    checked(
      await database
        .from("user_badges")
        .select(
          "id,earned_at,season_id,metadata,badges(id,key,name,description,family,rarity,icon_key,mark_asset_url,full_asset_url,series_key,series_order)",
        )
        .eq("user_id", user.id)
...
export async function equipProfileBadges(request: Request) {
  ...
  if (new Set(input.items.map((item) => item.slot)).size !== input.items.length)
    throw new ApiError(400, "INVALID_INPUT", "대표 훈장 슬롯을 확인해주세요.");
  checked(
    await adminDb().rpc("equip_badges", {
      p_user: user.id,
      p_items: input.items,
    }),
  );
